...
def get_best_combo(items, target_amount, max_qty, min_types, avoid_names=None):
    ...
    target_int = int(round(target_amount * 100)) # 放大100倍处理629.88这种两位小数
...
with col1:
    ...
    target_amount = st.number_input("目标报账总额 (元)", value=629.88, step=1.0) # 支持填 629.88
...
if not st.session_state.menu_df.empty:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            "锁定": st.column_config.CheckboxColumn(help="勾选后，重算时该菜品的数量和金额保持不变"),
